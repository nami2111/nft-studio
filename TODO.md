...
<Image src={cdnUrl} alt={nft.name} width={300} height={300} class="rounded-lg" />
```
...
```svelte
<img src="/api/images/{nft.id}?s=300&f=webp" alt={nft.name} width="300" height="300" />
```
...
<script lang="ts">
	...
	let filteredNFTs = $derived.by(() => {
		...
		return nfts.filter((nft) => nft.name.toLowerCase().includes(searchQuery.toLowerCase()));
	});
...
<SvelteVirtualList items={filteredNFTs} itemSize={250} overscan={10}>
...
const gridVirtualizer = combineVirtualizers(rowVirtualizer, columnVirtualizer);
```
...
const cache = {
	...
	async get(id: string): Promise<ImageData | null> {
		return this.l1.get(id) || (await this.l2.get(id)) || (await this.l3.get(id));
	}
};
```
