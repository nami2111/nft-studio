...
> Updated from codebase audit on 2026-06-20. Last review 2026-06-21.
...
>
> - Done: generation UI type fixes, completion ownership cleanup + tests, filename index parsing,
>   cancel-flow cleanup, verified dead-code deletion, scheduler `onMessage` removal,
>   worker-pool forwarded-message typing, dispatch-message rename alias, feature-flag normalization,
>   packaging/finalizing phase detection, generation validation consolidation, orchestration
>   completion/chunk/cancel tests (P1 #7), and worker logging policy (P2 #11 — `zip.worker.ts`
>   trace logs routed through DEV-gated `logger.debug`).
> - Remaining: CSP worker design (P3 #13, deferred until profiling confirms the need) and unrelated
>   existing `layer/` test failures (`indexedDB is not defined` in jsdom) outside the generation flow.
...
**Tests added** (in `src/lib/workers/generation.orchestrator.test.ts`):

- ✅ Worker `complete` with `isChunk: true` streams via `addStreamingChunk` but does not call user `onComplete` directly.
- ✅ Orchestrator calls `onComplete` exactly once after `streamer.finalize()` (happy path + chunk tests).
- ✅ Cancellation cancels the active session streamer and fires `onCancelled` exactly once.
- ✅ Chunk messages route to `streamBatch` when `enableStreamingStorage` is on, to `addStreamingChunk` otherwise.

**Deferred:**

- Storage streaming waits for queued writes before packaging — requires `streaming-storage` async
  ordering mocks; the chunk-routing test above covers the dispatch path.
...
- `src/lib/workers/zip.worker.ts` was the actual noisiest offender (12 unguarded `console.log` trace
  calls in prod). All converted to the existing `logger.debug` (DEV-gated). `console.warn` (empty
  files) and `console.error` (fatal failures) left visible per policy.
- Audited the other listed files:
  - `csp-solver.ts`, `generation.orchestrator.ts`, `trait-batch-scheduler.ts` — already use the
    `logger` / DEV-guarded pattern.
  - `pool/pool.ts`, `generation.worker.ts` — only `console.warn`/`console.error` remain, which is
    correct per the "keep unrecoverable errors visible" policy.
- `src/lib/utils/logger.ts` already implements the right policy (`info`/`debug` DEV-gated,
  `warn`/`error` always on) — no new helper needed.
