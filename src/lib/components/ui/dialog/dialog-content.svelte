...
<Dialog.Portal {...portalProps}>
	...
	<DialogPrimitive.Content
		...
		class={cn(
			'bg-white/95 border-gray-800 text-gray-900 shadow-2xl backdrop-blur-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid max-h-[85vh] w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto rounded-lg border p-6 duration-200 sm:max-w-lg',
			className
		)}
		aria-label={ariaLabel}
		aria-describedby={ariaDescribedBy}
