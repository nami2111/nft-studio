<script lang="ts">
    ...
    const MAX_TOTAL_SIZE = 5 * 1024 * 1024 * 1024; // 5GB total across all files

    // File size thresholds (in bytes)
    const SMALL_FILE_THRESHOLD = 200 * 1024 * 1024; // 200MB
    ...
    async function processFiles(files: File[]) {
        ...
        if (zipFiles.length === 0) {
            showWarning("No valid ZIP files", {
                description:
                    "Please select at least one ZIP file containing collections.",
            });
            return;
        }

        const oversizedFiles = zipFiles.filter(
            (f) => f.size > LARGE_FILE_THRESHOLD,
        );
        if (oversizedFiles.length > 0) {
            showWarning("Large files detected", {
                description: `${oversizedFiles.length} files exceed 5GB and will be skipped.`,
            });
        }

        const validFiles = zipFiles.filter(
            (f) => f.size <= LARGE_FILE_THRESHOLD,
        );
        ...
        try {
            ...
            for (let i = 0; i < validFiles.length; i++) {
                ...
                importProgress =
                    5 + Math.round(((i + 1) / validFiles.length) * 15);
            }
            ...
            if (metadataList.length === 0) {
                showError(new Error("No valid metadata found"), {
                    title: "Import Failed",
                    description:
                        "No valid JSON metadata was found in the ZIP files.",
                });
                return;
            }
            ...
            for (const { file, strategy, imageNames } of imageNamesByFile) {
                const onProgress = (count: number) => {
                    streamedCount += count;
                    const percent =
                        35 + Math.round((streamedCount / totalImages) * 60);
    ...
    ): Array<{ layer: string; trait: string; rarity: number }> {
        return attributes.map((attr) => ({
            layer:
                (attr.trait_type as string) ||
                (attr.layer as string) ||
                "Attribute",
...
<Card class="mx-auto max-w-2xl p-4 lg:max-w-3xl xl:max-w-4xl {className}">
    <div class="space-y-4">
        ...
        >
            <div class="space-y-2">
                ...
                <div class="text-muted-foreground text-xs">
                    Supports ZIP files exported from GNStudio
                </div>
                <div class="text-muted-foreground mt-1 text-xs">
                    Tip: You can select multiple ZIP files at once (Ctrl+Click
                    or Cmd+Click)
                </div>
                <div
        ...
        <div class="text-muted-foreground space-y-1 text-xs">
            <div>
                <strong>Important:</strong> Both folders must be at the root level
                inside the ZIP file
            </div>
