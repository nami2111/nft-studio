...
export async function prepareLayersForWorker(layers: Layer[]): Promise<TransferrableLayer[]> {
  ...
  for (const layer of layers) {
    if (layer.traits.length === 0) {
      throw new Error(`Layer "${layer.name}" has no traits. Please add at least one trait to each layer.`);
    }
...
export function getLayersWithMissingImages(layers: Layer[]): Array<{ layerName: string; traitName: string }> {
