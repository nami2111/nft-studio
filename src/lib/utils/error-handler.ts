 ...
 */
export function withSafeOperationSync<T>(
	operation: () => T,
	options: ErrorHandlerOptions = {},
): T {
	try {
...
export async function handleStorageError<T>(
	...
	return handleError<T>(storageError, {
		title: "Storage Error",
		description:
			"Failed to access storage. Please check your browser settings.",
		...options,
	});
}
...
export async function handleGenerationError<T>(
	...
	return handleError<T>(generationError, {
		title: "Generation Error",
		description:
			"Failed to generate items. Please check your project configuration.",
		...options,
	});
}
 ...
 */
export function getDetailedErrorInfo(
	error: unknown,
): ReturnType<typeof getErrorInfo> {
